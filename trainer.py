...
class Trainer(object):
    def __init__(self, arg):
        ...
        # self.Matcher = nn.DataParallel(self.Matcher, device_ids=[0,1,2,3], dim=0)
        self.batch_nums = 0
        if self.test:
            self.writer = None
        else:
    ...
    def train(self):
        ...
        for data in train_generate(self.dataset, self.batch_size, self.train_few, self.symbol2id, self.ent2id,
                                   self.e1rel_e2):
            support, support_negative, query, false, support_left, support_right, support_negative_left, support_negative_right, query_left, query_right, false_left, false_right = data

            self.batch_nums += 1
            support_meta = self.get_meta(support_left, support_right)
            support_negative_meta = self.get_meta(support_negative_left, support_negative_right)
            query_meta = self.get_meta(query_left, query_right)
            false_meta = self.get_meta(false_left, false_right)

            support = Variable(torch.LongTensor(support)).to(self.device)
            support_negative = Variable(torch.LongTensor(support_negative)).to(self.device)
            query = Variable(torch.LongTensor(query)).to(self.device)
            false = Variable(torch.LongTensor(false)).to(self.device)
            self.Matcher.train()

            # generate label
            support_label = torch.zeros(support.size(0))
            support_negative_label = torch.ones(support_negative.size(0))
            support_full_label = torch.cat((support_label, support_negative_label),0).unsqueeze(0)
            query_label = torch.zeros(query.size(0)).unsqueeze(1)
            false_label = torch.ones(false.size(0)).unsqueeze(1)
            query_full_label = torch.cat((query_label,false_label),0)
            support_full_label = support_full_label.repeat(query_label.size(0)+false_label.size(0),1)
            full_label = torch.cat((support_full_label,query_full_label),1)

            full_edge = self.label2edge(full_label)

            evaluation_mask = torch.ones(full_edge.size(0), full_edge.size(1), full_edge.size(1)).to(self.device)

            init_edge = full_edge.clone()
            init_edge[:, :, support.size(0)*2:, :] = 0.5
            init_edge[:, :, :, support.size(0)*2:] = 0.5
            for i in range(1):
                init_edge[:, 0, support.size(0)*2 + i, support.size(0)*2 + i] = 1.0
                init_edge[:, 1, support.size(0)*2 + i, support.size(0)*2 + i] = 0.0

            if self.no_meta:
                positive_score, negative_score, adj, n_positive_score, n_negative_score = self.Matcher(support, support_negative, query, false, isEval=False, init_adj=init_edge)
            else:
                positive_score, negative_score, adj, n_positive_score, n_negative_score = self.Matcher(support, support_negative, query, false, isEval=False,
                                                              init_adj=init_edge,
                                                              support_meta=support_meta,
                                                              support_negative_meta=support_negative_meta,
                                                              query_meta=query_meta,
                                                              false_meta=false_meta)
            margin_ = positive_score - negative_score
            margin_2 = n_negative_score - n_positive_score
            query_socre_loss = F.relu(self.margin - margin_).mean()
            query_score_loss_2 = F.relu(self.margin - margin_2).mean()
            margins.append(margin_.mean().item())
            full_edge = full_edge[:,0,:,:].squeeze()
            # bce_loss = self.bce_loss(adj,full_edge)
    ...
    def eval(self, mode='dev', meta=False):
        ...
        for query_ in test_tasks.keys():
            ...
            support_negative_pairs = []
            support_negative_left = []
            support_negative_right = []
            shift = 0

            for support_triple in support_triples:
                support_noise_list = []
                s_h = support_triple[0]
                rel = support_triple[1]
                s_t = support_triple[2]
                while True:
                    support_noise = candidates[shift]
                    if (support_noise not in self.e1rel_e2[s_h + rel]) and support_noise != s_t:
                        support_noise_list.append(support_noise)
                        break
                    else:
                        shift += 1
                support_negative_pairs.append([symbol2id[s_h], symbol2id[support_noise]])
                support_negative_left.append(self.ent2id[s_h])
                support_negative_right.append(self.ent2id[support_noise])

            if meta:
                support_negative_meta = self.get_meta(support_negative_left, support_negative_right)

            support_negative = Variable(torch.LongTensor(support_negative_pairs)).to(self.device)
            ...
            for triple in test_tasks[query_][few:]:
                ...
                support_negative_label = torch.ones(support_negative.size(0))
                support_full_label = torch.cat((support_label, support_negative_label), 0).unsqueeze(0)
                query_label = torch.zeros(1).unsqueeze(1)
                false_label = torch.ones(query.size(0)-1).unsqueeze(1)
                query_full_label = torch.cat((query_label, false_label), 0)
                support_full_label = support_full_label.repeat(query_label.size(0) + false_label.size(0), 1)
                full_label = torch.cat((support_full_label, query_full_label), 1)
                ...
                with torch.no_grad():
                    if meta:
                        query_meta = self.get_meta(query_left, query_right)
                        scores, _, adj, n_positive_score, n_negative_score = self.Matcher(support, support_negative, query, None, isEval=True,
                                                 init_adj = init_edge,
                                                 support_meta=support_meta,
                                                 support_negative_meta=support_negative_meta,
                                                 query_meta=query_meta,
